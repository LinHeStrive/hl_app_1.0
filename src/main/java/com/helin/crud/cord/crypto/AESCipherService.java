package com.helin.crud.cord.crypto;

import com.helin.crud.constant.AESConstants;
import com.helin.crud.util.ArrayUtil;
import com.helin.crud.util.Base64Util;
import static com.helin.crud.util.ArrayUtil.transferToShorts;
...
//        printRoundKeys(rawRoundKeys);
	...
	private void printRoundKeys(short[][] roundKeys) {
		for (int i = 0, keyOrder = 1; i < roundKeys.length; i += 4, keyOrder++) {
			String infoKValue = getStateHex(
					new short[][] { roundKeys[i], roundKeys[i + 1], roundKeys[i + 2], roundKeys[i + 3] });
